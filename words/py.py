...
open_diff = open('E:/BaiNetDiskDL/Try/Time_Waifu-main/waifuTime/words/all.txt', 'r',encoding='ANSI') # 源文本文件
...
for i,j in zip(range(0,int(count/n+1)),range(0,int(count/n+1))): # 写入txt，计算需要写j入的文件数
    with open('E:/BaiNetDiskDL/Try/Time_Waifu-main/waifuTime/words/%d.txt'%(j),'w+') as temp:
